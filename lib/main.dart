...
class _NucleusUniverseScreenState extends State<NucleusUniverseScreen> {
  ...
  // When set (while paused), this overrides the engine's state for display.
  NucleusState? _selectedNucleus;
  ...
  void _reset() {
    ...
    _selectedNucleus = null; // clear manual selection
    _inner.reset(maxZ: allElements.length);
    setState(() {});
  }
  ...
  void _start() {
    if (_running) return;
    _selectedNucleus = null; // engine takes over again
    _running = true;
  ...
  void _step() {
    _selectedNucleus = null; // each step returns to live engine state
    setState(() {
  ...
  @override
  Widget build(BuildContext context) {
    final nucleus = _selectedNucleus ?? _inner.state;
    final judgeSnapshot =
                        ...
                        child: Padding(
                          ...
                          child: _PeriodicTablePanel(
                            ...
                            onElementTap: (element) {
                              if (_running)
                                return; // only interactive while running is false

                              setState(() {
                                final int z = element.Z;
                                // Simple heuristic: start on N ≈ Z (roughly along valley of stability)
                                final int n = z;
                                _selectedNucleus = NucleusState(
                                  protons: z,
                                  neutrons: n,
                                );
                              });
                        ...
                        style: ElevatedButton.styleFrom(
                          backgroundColor:
                              _running
                                  ...
                                  : Colors.tealAccent.withOpacity(0.32),
                          foregroundColor: Colors.tealAccent,
                          padding: const EdgeInsets.symmetric(
                            horizontal: 24,
                            vertical: 12,
  ...
  Widget _buildInfoRow() {
    final n = _selectedNucleus ?? _inner.state;
...
class _PeriodicTablePanel extends StatelessWidget {
  ...
  final int currentZ;
  final void Function(ElementDef element)? onElementTap;
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        // Phone portrait etc. → treat as compact
        final bool isCompact = cellHeight < 32;

        return GestureDetector(
          behavior: HitTestBehavior.opaque,
          onTapDown: (details) {
            if (onElementTap == null) return;

            final Offset localPos = details.localPosition;

            final int col = (localPos.dx / cellWidth).floor();
            final int row = (localPos.dy / cellHeight).floor();

            if (row < 0 || row >= rows || col < 0 || col >= cols) return;

            final String key = '${row + 1}-${col + 1}';
...
class _PeriodicTablePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Slightly smaller fonts on compact layouts
    final double symbolFontSize =
        cellHeight * (isCompact ? 0.22 : 0.26); // was 0.26
    final double zFontSize = cellHeight * (isCompact ? 0.12 : 0.16); // was 0.16
    ...
    for (int r = 0; r < rows; r++) {
      for (int c = 0; c < cols; c++) {
        ...
        if (element != null) {
          ...
          // Position Z in the top-left, symbol lower/centred → no overlap
          final symbolY =
              rect.top +
              rect.height * (isCompact ? 0.45 : 0.30) -
              symbolPainter.height / 2;

          symbolPainter.paint(
            canvas,
            Offset(rect.left + (rect.width - symbolPainter.width) / 2, symbolY),
          );
